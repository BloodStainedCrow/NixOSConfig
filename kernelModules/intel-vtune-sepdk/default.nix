...
stdenv.mkDerivation {
  ...
  makeFlags = [
    ...
    "KERNEL_HEADER_DIR=${kernel.dev}/lib/modules/${kernel.modDirVersion}/source"
  ...
  preBuild = ''
    ...
    # ls -la ${kernel.dev}/lib/modules/${kernel.modDirVersion}/source/include
  '';
  ...
  postInstall = ''
    ...
    for i in insmod-sep pax/insmod-pax \
             rmmod-sep  pax/rmmod-pax; do
      sed -i '
        s/^PATH=/# &/;
